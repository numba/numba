...
if [ "$RUN_COVERAGE" == "yes" ]; then
    ...
    NUMBA_USE_TYPEGUARD=0 NUMBA_ENABLE_CUDASIM=1 PYTHONWARNINGS="ignore:::typeguard" $SEGVCATCH python runtests.py -b -j "$TEST_START_INDEX:$TEST_COUNT" --exclude-tags='long_running' -m $TEST_NPROCS -- numba.tests
else
