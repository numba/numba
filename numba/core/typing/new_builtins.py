...
class Abs(ConcreteTemplate):
    cases = []
    # Python cases
    cases += [signature(op, op) for op in [types.py_int64]]
    cases += [signature(op, op) for op in types.py_real_domain]
    cases += [signature(op.underlying_float, op) for op in types.py_complex_domain]
...
class Slice(ConcreteTemplate):
    cases = [
        # Python Types
        signature(types.slice2_type, types.py_intp),
        signature(types.slice2_type, types.none),
        signature(types.slice2_type, types.none, types.none),
        signature(types.slice2_type, types.none, types.py_intp),
        signature(types.slice2_type, types.py_intp, types.none),
        signature(types.slice2_type, types.py_intp, types.py_intp),
        signature(types.slice3_type, types.py_intp, types.py_intp, types.py_intp),
        signature(types.slice3_type, types.none, types.py_intp, types.py_intp),
        signature(types.slice3_type, types.py_intp, types.none, types.py_intp),
        signature(types.slice3_type, types.py_intp, types.py_intp, types.none),
        signature(types.slice3_type, types.py_intp, types.none, types.none),
        signature(types.slice3_type, types.none, types.py_intp, types.none),
        signature(types.slice3_type, types.none, types.none, types.py_intp),
...
class Range(ConcreteTemplate):
    cases = [
        ...
        signature(types.range_state64_type, types.py_int64, types.py_int64, types.py_int64),
        
        # NumPy Types
        signature(types.range_state64_type, types.np_int64),
        signature(types.range_state64_type, types.np_int64, types.np_int64),
        signature(types.range_state64_type, types.np_int64, types.np_int64, types.np_int64),
        
        # Python + NumPy Types
        signature(types.range_state64_type, types.py_int64, types.np_int64),
        signature(types.range_state64_type, types.np_int64, types.py_int64),

        signature(types.range_state64_type, types.py_int64, types.np_int64, types.np_int64),
        signature(types.range_state64_type, types.np_int64, types.py_int64, types.np_int64),
        signature(types.range_state64_type, types.np_int64, types.np_int64, types.py_int64),
        signature(types.range_state64_type, types.py_int64, types.py_int64, types.np_int64),
        signature(types.range_state64_type, types.py_int64, types.np_int64, types.py_int64),
        signature(types.range_state64_type, types.np_int64, types.py_int64, types.py_int64),
    ]
...
# TODO: Generalize this to use NEP-50 and calculate result beyond integer cases
def choose_result_int(arg_1, other):
    """
    ...
    type system.
    """
    # If everything is a NumPy integer, promotion should be to the
    # respective NumPy type.
    if all('np' in tp.name for tp in (arg_1, other)):
        bitwidth = max(arg_1.bitwidth, other.bitwidth)
        signed = arg_1.signed or other.signed
        return types.NumPyInteger.from_bitwidth(bitwidth, signed)

    if any('np' in tp.name for tp in (arg_1, other)):
        # This means one of the integer is numpy so NEP-50 applies
        # and we should return the numpy integer type.
        if 'np' in arg_1.name:
            return arg_1
        else:
            return other
...
py_ints = (types.py_int64,)
np_ints = (types.np_int32, types.np_int64, types.np_uint32, types.np_uint64)


def get_mixture_result_types(num_args, arg_types, res_type_func):
    """
    Get the result type of a signature input of mixture of types
    using the res_type_func.
    """
    all_cases = itertools.product(arg_types, repeat=num_args)
    return [signature(res_type_func(*case), *case) for case in all_cases]
...
class BinOp(ConcreteTemplate):
    ...
    # TODO: Think about mixture float and complex cases
    pass
...
def py_bool__add__(self, other):
    def impl(self, other):
        if isinstance(other, bool) and hasattr(other, "__bool__"):
            asbool = other.__bool__()
            if asbool is NotImplemented:
                return NotImplemented
            else:
                return bool_add_bool(self, asbool)
        elif isinstance(other, int) and hasattr(self, "__int__"):
            asint = self.__int__()
            if asint is NotImplemented:
                return NotImplemented
            else:
                return int_add_int(asint, other)
        else:
            return NotImplemented
    return impl
...
def py_int__add__(self, other):
    # evaluate type
    def impl(self, other):
        if isinstance(other, (bool, int)) and hasattr(other, "__int__"):
            asint = other.__int__()
            if asint is NotImplemented:
                return NotImplemented
            else:
                return int_add_int(self, asint)
        else:
            return NotImplemented
    return impl
...
def py_float__add__(self, other):
    ...
    def impl(self, other):
        if hasattr(other, "__float__"):
            asfloat = other.__float__()
            if asfloat is NotImplemented:
                return NotImplemented
            else:
...
def py_complex__add__(self, other):
        ...
        def impl(self, other):
            if hasattr(other, "__float__"):
                as_float = other.__float__()
                if as_float is NotImplemented:
                    return NotImplemented
                else:
                    return complex_add_complex(self, complex(as_float))
            elif hasattr(other, "__complex__"):
                as_complex = other.__complex__()
                if as_complex is NotImplemented:
                    return NotImplemented
                else:
                    return complex_add_complex(self, as_complex)
            else:
                return NotImplemented
        return impl
...
def generate_binop(op_func, slot, rslot, opchar):
    ...
    def ol_binary_op_number(v, w):
        if not isinstance(v, (types.Number, types.Boolean)) or not isinstance(w, (types.Number, types.Boolean)):
            return

        # this really needs to only deal with number and sequence types, can
        # either make unsupported typse a compilation error, or fall back to
        # making it a runtime error like "unsupported operand for types..."
...
class BitwiseShiftOperation(ConcreteTemplate):
    ...
    cases = [signature(types.py_intp, op, op)
             for op in sorted(types.py_signed_domain)]
...
class Complex(AbstractTemplate):
    ...
    def generic(self, args, kws):
        assert not kws
        for arg in args:
            if arg not in (types.py_number_domain | types.np_number_domain):
                raise errors.NumbaTypeError("complex() only support for numbers")

        return signature(types.py_complex128, *args)
