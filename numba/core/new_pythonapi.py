...
class PythonAPI(object):
    ...
    def list_getitem(self, lst, idx):
        ...
        if isinstance(idx, int):
            idx = self.context.get_constant(types.c_intp, idx)
    ...
    def tuple_pack(self, items):
        ...
        n = self.context.get_constant(types.c_intp, len(items))
        args = [n]
        args.extend(items)
    ...
    def object_richcompare(self, lhs, rhs, opstr):
        ...
        if opstr in ops:
            ...
            lopid = self.context.get_constant(types.c_int32, opid)
    ...
    def nrt_adapt_ndarray_to_python(self, aryty, ary, dtypeptr):
        ...
        ndim = self.context.get_constant(types.c_int32, aryty.ndim)
        writable = self.context.get_constant(types.c_int32, int(aryty.mutable))
    ...
    def list_pack(self, items):
        n = len(items)
        seq = self.list_new(self.context.get_constant(types.c_intp, n))
        with self.if_object_ok(seq):
            for i in range(n):
                idx = self.context.get_constant(types.c_intp, i)
                self.incref(items[i])
    ...
    def string_from_constant_string(self, string):
        ...
        sz = self.context.get_constant(types.c_intp, len(string))
