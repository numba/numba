...
from numba.core import (
    utils, types, errors, typing, serialize, config, compiler, sigutils, ir
)
...
class TypeFolder:
    def normal_handler(self, index: int, param: inspect.Parameter,
                       value: types.Type):
        return value

    def default_handler(self, index: int, param: inspect.Parameter,
                        default: object):
        return types.Omitted(default)

    def stararg_handler(self, index: int, param: inspect.Parameter,
                        values: pt.Tuple[types.Type, ...]):
        # NOTE: avoid wrapping the tuple type for stararg in another
        #       tuple.
        if len(values) == 1:
            stararg_types = (types.StarArgTuple, types.StarArgUniTuple)
            if isinstance(values[0], stararg_types):
                return values[0]
            ...
            with self._compiling_counter:
                ...
                with ev.trigger_event("numba:compile", data=ev_details):
                    try:
                        ...
                        def folder(hdlr):
                            return FoldArguments(self._compiler.pysig, hdlr)
                        raise e.bind_folder(folder)
                    self.add_overload(cres)
