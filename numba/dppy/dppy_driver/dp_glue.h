...
typedef struct dp_glue_env* env_t;


struct dp_glue_buffer
{
    unsigned id_;
...
typedef struct dp_glue_buffer* buffer_t;


struct dp_glue_kernel
{
    unsigned id_;
    void *kernel;
    int (*dump_fn) (void *);
...
typedef struct dp_glue_kernel* kernel_t;


struct dp_glue_program
{
    unsigned id_;
    void *program;
};

typedef struct dp_glue_program* program_t;


struct dp_glue_kernel_arg
{
    unsigned id_;
...
typedef struct dp_glue_kernel_arg* kernel_arg_t;


/*! @struct dp_glue_runtime_t
 *  @brief Stores an array of the available OpenCL or Level-0 platform/drivers.
 *
 *  @var dp_glue_runtime_t::num_platforms
 *  Depicts the number of platforms/drivers available on this system
 *
 *  @var dp_glue_runtime_t::platforms_ids
...
typedef struct dp_glue_runtime* runtime_t;
 ...
 *
 */
int destroy_dp_glue_program (program_t *program_t_ptr);
