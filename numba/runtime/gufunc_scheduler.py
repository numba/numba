...
def create_schedule(full_space, num_sched):
    ...
    if full_space.ndim() == 1:
        ra_len = ipd[0]
        if ra_len <= num_sched:
            return np.array([RangeActual(full_space.start[0]+x,full_space.start[0]+x) if x < ra_len else RangeActual(-1,-2) for x in range(num_sched)])
        else:
            ilen = ra_len // num_sched
            imod = ra_len %  num_sched 
            return np.array([RangeActual(full_space.start[0] + (ilen * x), full_space.start[0] + ((ilen * (x+1))) - 1 if x < (num_sched-1) else full_space.end[0]) for x in range(num_sched)])
    else:
