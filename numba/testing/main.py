    ...
    * a -l option to (only) list tests
    ...
    def _getParentArgParser(self):
        ...
        parser.add_argument(
            ...
            dest="show_timing",
            action="store_true",
            help=("Print recorded timing for each test and " "the whole suite."),
        )
        parser.add_argument(
            "--junit",
            dest="write_junit",
...
class ParallelTestResult(runner.TextTestResult):
    ...
    def write_junit_xml(self, output):
        ...
        for rec in self.records:
            ...
            SubElement(
                props, "property", name="start_time", value=str(rec["start_time"])
            )
...
class _MinimalRunner(object):
    ...
    def __init__(
        self, runner_cls, runner_args, *, show_timing=False, write_junit=False
    ):
...
class ParallelTestRunner(runner.TextTestRunner):
    ...
    def __init__(
        self, runner_cls, nprocs, useslice, *, show_timing, write_junit, **kwargs
    ):
        runner.TextTestRunner.__init__(self, **kwargs)
        self.runner_cls = runner_cls
        self.nprocs = nprocs
    ...
    def _run_inner(self, result):
        ...
        child_runner = _MinimalRunner(
            self.runner_cls, self.runner_args, show_timing=self.show_timing
        )
        ...
        with multiprocessing.Manager() as manager:
            for tests in splitted_tests:
                ...
                queue = manager.Queue()
                try:
                    self._run_parallel_tests(result, pool, child_runner, tests, queue)
                except:
    ...
    def _run_parallel_tests(self, result, pool, child_runner, tests, queue):
        ...
        while True:
            try:
                ...
                msgbuf = ["Active tests didn't finish before timeout (or crashed):"]
                active_pids = []
