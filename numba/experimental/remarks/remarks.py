...
def render_file_source(context, source_dir, output_dir, filename, line_remarks):
    ...
    def render_source_lines(stream, line_remarks):
        ...
        for (linenum, html_line) in enumerate(
                html_highlighted.split('\n'), start=1
        ):
    ...
    <h1>Unable to locate file {filename}</h1>
        ...
        try:
            with open(filename, encoding="utf8",
                      errors='ignore') as source_stream:
...
class RemarksInterface:
    ...
    def render_remarks(self, output_dir, source_dir,
                       open_browser=False, num_jobs=1):
        pathlib.Path(output_dir).mkdir(parents=True, exist_ok=True)

        logging.info('Rendering index page...')
        logging.info(f"  {len(self.all_remarks):d} raw remarks")
        if len(self.all_remarks) == 0:
            logging.warning("Not generating report! Please verify your"
                            " --source-dir argument is exactly the path"
                            " from which the compiler was invoked.")
            return

        sorted_remarks = sorted(self.all_remarks.values(),
                                key=lambda r: (r.File, r.Line,
                                               r.Column, r.PassWithDiffPrefix))
        unique_lines_remarks = [sorted_remarks[0]]
        for rmk in sorted_remarks:
            last_unq_rmk = unique_lines_remarks[-1]
            last_rmk_key = (last_unq_rmk.File, last_unq_rmk.Line,
                            last_unq_rmk.Column,
                            last_unq_rmk.PassWithDiffPrefix)
            rmk_key = (rmk.File, rmk.Line, rmk.Column, rmk.PassWithDiffPrefix)
            if rmk_key != last_rmk_key:
                unique_lines_remarks.append(rmk)
        logging.info("  {:d} unique source locations".format(
            len(unique_lines_remarks))
        )

        if self.should_display_hotness:
            sorted_remarks = sorted(unique_lines_remarks, key=lambda r:
                                    (r.Hotness, r.File, r.Line, r.Column,
                                     r.PassWithDiffPrefix, r.yaml_tag,
                                     r.Function),
                                    reverse=True)
        else:
            sorted_remarks = sorted(unique_lines_remarks, key=lambda r:
        ...
        for filename, entry in self.file_remarks.items():
            render_file_source(self.context, source_dir,
                               output_dir, filename, entry)
