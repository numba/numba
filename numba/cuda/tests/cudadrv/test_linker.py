...
from numba import cuda, void, float64, int64, int32
...
def simple_smem(ary):
    sm = cuda.shared.array(100, int32)
    i = cuda.grid(1)
    if i == 0:
        for j in range(100):
            sm[j] = j
...
class TestLinker(CUDATestCase):
    ...
    def test_get_shared_mem_per_block2(self):
        compiled = cuda.jit(simple_smem)
        compiled_specialized = compiled.specialize(np.zeros(200, dtype=np.int32))
