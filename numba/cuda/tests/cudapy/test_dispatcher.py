...
class TestDispatcherSpecialization(CUDATestCase):
    ...
    def test_specialize_adds_overload(self):
        # Ensure that specializing a dispatcher adds the overload generated by
        # specializing the dispatcher to the generic dispatcher, so we don't
        # need to generate the overload on multiple uses.
        @cuda.jit
        def f(x, y):
            pass

        self.assertEqual(len(f.overloads), 0)

        # 'F' order specialization
        f_arg = np.zeros((2, 2), order='F')
        f_arg_ty = typeof(f_arg)
        f_f32a_f32a = f.specialize(f_arg, f_arg)
        self.assertEqual(len(f.overloads), 1)
        self.assertIs(f_f32a_f32a.overloads[f_arg_ty, f_arg_ty],
                      f.overloads[f_arg_ty, f_arg_ty])

        # 'C' order specialization
        c_arg = np.zeros((2, 2), order='C')
        c_arg_ty = typeof(c_arg)
        f_f32c_f32c = f.specialize(c_arg, c_arg)
        self.assertEqual(len(f.overloads), 2)
        self.assertIs(f_f32c_f32c.overloads[c_arg_ty, c_arg_ty],
                      f.overloads[c_arg_ty, c_arg_ty])

        # Reuse 'C' order specialization
        f_f32c_f32c_2 = f.specialize(c_arg, c_arg)
        self.assertEqual(len(f.overloads), 2)
        self.assertIs(f_f32c_f32c_2.overloads[c_arg_ty, c_arg_ty],
                      f.overloads[c_arg_ty, c_arg_ty])
