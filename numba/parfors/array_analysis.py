...
class ArrayAnalysis(object):
    ...
    def gen_literal_slice_part(
        ...
        static_literal_slice_part_var = scope.make_temp(loc, name)
    ...
    def gen_explicit_neg(
        ...
        # Create var to hold the calculated slice size.
        explicit_neg_var = scope.make_temp(loc, "explicit_neg")
    ...
    def slice_size(self, index, dsize, equiv_set, scope, stmts):
        ...
        size_var = scope.make_temp(loc, "slice_size")
        ...
        wrap_var = scope.make_temp(loc, "wrap")
        ...
        post_wrap_size_var = scope.make_temp(loc, "post_wrap_slice_size")
    ...
    def _index_to_shape(self, scope, equiv_set, var, ind_var):
        ...
        if replace_index:
            ...
            if len(index_var_list) > 1:
                ...
                replacement_build_tuple_var = scope.make_temp(
                    ind_shape[0].loc, "replacement_build_tuple")
    ...
    def _analyze_op_call_numpy_reshape(self, scope, equiv_set, loc, args, kws):
        ...
        if neg_one_index >= 0:
            ...
            calc_size_var = scope.make_temp(loc, "calc_size_var")
            ...
            for arg_index in range(1, len(args)):
                ...
                if arg_index == neg_one_index:
                    continue
                div_calc_size_var = scope.make_temp(loc, "calc_size_var")
    ...
    def _make_assert_equiv(self, scope, loc, equiv_set, _args, names=None):
        ...
        msg_var = scope.make_temp(loc, "msg")
        ...
        assert_var = scope.make_temp(loc, "assert")
    ...
    def _gen_shape_call(self, equiv_set, var, ndims, shape, post):
        ...
        if isinstance(shape, ir.Var):
            ...
            attr_var = var.scope.make_temp(var.loc, "{}_shape".format(var.name))
        ...
        for i in range(ndims):
            ...
            if shape and shape[i]:
                if isinstance(shape[i], ir.Var):
                    ...
                    size_var = var.scope.make_temp(
                        var.loc, "{}_size{}".format(var.name, i))
            ...
            if not skip:
                ...
                size_var = var.scope.make_temp(var.loc,
                                               "{}_size{}".format(var.name, i))
