...
def arange_parallel_impl(return_type, *args):
    ...
    if len(args) == 1:
        ...
        raise ValueError(f"parallel arange with types {args}")
...
def linspace_parallel_impl(return_type, *args):
    ...
    if len(args) == 2:
        ...
        raise ValueError(f"parallel linspace with types {args}")
...
class Parfor(ir.Expr, ir.Stmt):
    ...
    def dump(self, file=None):
        ...
        print((f"begin parfor {self.id}").center(20, '-'), file=file)
...
class ParforDiagnostics:
                ...
                if reported_loc.filename == filename:
                    ...
                    for adj in graphs:
                        if adj: # graph may be empty, e.g. no nesting
                            for k in adj[pf_id]:
                                tmp.append(self.sort_pf_by_line(
                                    k, parfors_simple))
                            if tmp:
            ...
            if isinstance(pattern, tuple):
                if pattern[0] == 'prange':
                    if pattern[1] == 'internal':
                        ...
                        loc = pattern[2][1]
                        r_pattern = '{} {}'.format(
                            replfn, '(internal parallel version)')
                    elif pattern[1] == 'user':
    ...
    def source_listing(self, parfors_simple, purpose_str):
        ...
        if lines and parfors_simple:
            ...
            max_pf_per_line = max([1] + [len(x)
                                  for x in map_line_to_pf.values()])
    ...
    def print_unoptimised(self, lines):
        ...
        def print_nest(fadj_, nadj_, theroot, reported, region_id):
            def print_g(fadj_, nadj_, nroot, depth):
                print_wrapped(fac * depth * ' ' + '%s%s %s' %
                              (sword, nroot, '(parallel)'))
                for k in nadj_[nroot]:
                    if nadj_[k] == []:
                        msg = []
                        msg.append(fac * (depth + 1) * ' ' + '%s%s %s' %
                                   (sword, k, '(parallel)'))
                        if fadj_[k] != [] and k not in reported:
                            fused = self.reachable_nodes(fadj_, k)
                            for i in fused:
                                msg.append(fac * (depth + 1) * ' ' +
                                           '{}{} {}'.format(sword, i, '(parallel)'))
                        reported.append(k)
                        print_wrapped('\n'.join(msg))
                    else:
        ...
        def print_fuse(ty, pf_id, adj, depth, region_id):
            msg = []
            print_wrapped("Parallel region %s:" % region_id)
            msg.append(fac * depth * ' ' + '%s%s %s' %
                       (sword, pf_id, '(parallel)'))
            if adj[pf_id] != []:
    ...
    def print_optimised(self, lines):
        ...
        def print_nest(fadj_, nadj_, theroot, reported, region_id):
            ...
            if nadj_[theroot] != []:
                print_wrapped("Parallel region %s:" % region_id)
                print_wrapped('{}{} {}'.format(sword, theroot, '(parallel)'))
                summary[region_id] = {'root': theroot,
                                      'fused': 0, 'serialized': 0}
        ...
        def print_fuse(ty, pf_id, adj, depth, region_id):
            ...
            summary[region_id] = {'root': pf_id,
                                  'fused': len(fused), 'serialized': 0}
            msg += ')'
            print_wrapped(msg)
            print("\n")
            region_id = region_id + 1
                    ...
                    try:
                        ...
                        if attr == 'empty':
                            ...
                            if lines and loc.line:
                                print_wrapped(
                                    "   Allocation:: " + lines[0 if loc.line < 2 else loc.line - 1].strip())
                            print_wrapped(
    ...
    def dump(self, level=1):
        ...
        print_wrapped((" Parallel Accelerator Optimizing: %s " %
                      purpose_str).center(_termwidth, '='))
        print_wrapped(_termwidth * "=")
        print_wrapped("")
        ...
        if n_parfors > -1:
            ...
            if self.fusion_info != {}:
                if print_fusion_summary:
                    ...
                    dump_graph_indented(
                        self.fusion_info, 'has the following loops fused into it:', '(fused)')
...
#----------- LICM section
            ...
            print_wrapped(
                f'Function %s, %s, has no parallel for-loops.')
                ...
                if isinstance(instr, ir.Assign):
                    ...
                    if isinstance(expr, ir.Expr) and expr.op == 'call':
                        ...
                        def replace_func():
                            ...
                            typs = tuple(self.typemap[x.name]
                                         for x in expr.args)
                            try:
...
class ConvertNumpyPass:
    ...
    def _arrayexpr_to_parfor(self, equiv_set, lhs, arrayexpr, avail_vars):
        ...
        pat = (f'array expression {repr_arrayexpr(arrayexpr.expr)}',)

        parfor = Parfor(loopnests, init_block, {}, loc, index_var,
                        equiv_set, pat[0], pass_states.flags)
    ...
    def _numpy_map_to_parfor(self, equiv_set, call_name, lhs, args, kws, expr):
        ...
        if call_name == 'zeros':
            ...
            raise NotImplementedError(
                f"Map of numpy.{call_name} to parfor is not implemented")
        ...
        parfor = Parfor(loopnests, init_block, {}, loc, index_var, equiv_set,
                        (f'{call_name} function', 'NumPy mapping'),
                        pass_states.flags)
        parfor.loop_body = {body_label: body_block}
        if config.DEBUG_ARRAY_OPT >= 1:
...
class ConvertReducePass:
    ...
    def _reduce_to_parfor(self, equiv_set, lhs, args, loc):
        ...
        parfor = Parfor(loopnests, init_block, loop_body, loc, index_var,
                        equiv_set, (f'{call_name} function',
...
class ConvertLoopPass:
        ...
        for loop, s in sorted(sized_loops, key=lambda tup: tup[1]):
            ...
            for inst in blocks[entry].body:
                ...
                if (isinstance(inst, ir.Assign)
                    ...
                    header_body = header_body[:li_index] + \
                        header_body[li_index + 1:]
                    ...
                    cps, _ = get_block_copies({0: blocks[loop.header]},
                                              pass_states.typemap)
                    cps = cps[0]
                    loop_index_vars = {t for t, v in cps if v == loop_index}
                    ...
                    def find_indexed_arrays():
                        ...
                        for block in loop_body.values():
                            for inst in block.body:
                                lv = {x.name for x in inst.list_vars()}
                                if lv & indices:
                    ...
                    def find_mask_from_size(size_var):
                        ...
                        live_vars = set.union(*[live_map[l]
                                              for l in loop.exits])
                    ...
                    if loop_kind == 'pndindex':
                        ...
                        loop_body[first_body_label].body = header_body + \
    ...
    def _get_loop_kind(self, func_var, call_table):
        ...
        if call[0] == 'internal_prange' or call[0] == internal_prange:
            try:
                kind = 'internal', (
                    pass_states.swapped_fns[func_var][0], pass_states.swapped_fns[func_var][-1])
            except KeyError:
...
class ParforPass(ParforPassStates):
    ...
    def run(self):
        ...
        simplify(self.func_ir, self.typemap,
                 self.calltypes, self.metadata["parfors"])
            ...
            for (block_label, block) in self.func_ir.blocks.items():
                ...
                for stmt in block.body:
                    ...
                    if isinstance(stmt, ir.Assign):
                        ...
                        rhs = stmt.value
                        lhs_typ = self.typemap[lhs.name]
                        print("Adding print for assignment to ",
                              lhs.name, lhs_typ, type(lhs_typ))
        ...
        if sequential_parfor_lowering:
            ...
            if config.DEBUG_ARRAY_OPT_STATS:
                ...
                if n_parfors > 0:
                    ...
                    print(f'Function {name} has no Parfor.')
...
def _gen_arrayexpr_getitem(
    ...
    if ndims == 0:
        ...
        stmts = ir_utils.gen_np_call('ravel', numpy.ravel, ravel_var, [
                                     var], typingctx, typemap, calltypes)
...
def get_reduction_init(nodes):
    ...
    acc_expr = list(filter(lambda x: isinstance(
        x.value, ir.Expr), nodes))[-1].value
...
def get_reduce_nodes(reduction_node, nodes, func_ir):
    ...
    for i, stmt in enumerate(nodes):
        ...
        if isinstance(rhs, ir.Expr):
            in_vars = {lookup(v, True).name for v in rhs.list_vars()}
            if name in in_vars:
...
def get_expr_args(expr):
    ...
    raise NotImplementedError(f"get arguments for expression {expr}")
...
def try_fuse(equiv_set, parfor1, parfor2, metadata, func_ir, typemap):
    ...
    for i in range(ndims):
        ...
        if not (is_equiv(nest1.start, nest2.start) and
            ...
            msg += "slice({}, {}, {}) != ".format(get_user_varname(nest1.start),
                                                  get_user_varname(nest1.stop), get_user_varname(nest1.step))
            msg += "slice({}, {}, {})".format(get_user_varname(nest2.start),
    ...
    if not p1_cross_dep:
        p2_cross_dep = has_cross_iter_dep(
            parfor2, func_ir, typemap, p1_ip, p1_ia, p1_non_ia)[0]
    else:
        p2_cross_dep = True
...
def fuse_parfors_inner(parfor1, parfor2):
    ...
    nameset = {x.name for x in index_dict.values()}
...
def remove_dead_parfor_recursive(parfor, lives, arg_aliases, alias_map,
    ...
    branchcond = ir.Var(scope, mk_unique_var(
        "$branchcond"), ir.Loc("parfors_dummy", -1))
    typemap[branchcond.name] = types.boolean

    branch = ir.Branch(branchcond, first_body_block,
                       return_label, ir.Loc("parfors_dummy", -1))
    ...
    for block in blocks.values():
        ...
        for stmt in block.body:
            ...
            if not nested and isinstance(stmt, Parfor):
                ...
                push_call_vars(pblocks, saved_globals,
                               saved_getattrs, typemap, nested=True)
                new_body.append(stmt)
                continue
            else:
                process_assign(stmt)
...
def repr_arrayexpr(arrayexpr):
    ...
    if isinstance(arrayexpr, tuple):
        ...
        args = arrayexpr[1]
        if len(args) == 1:
            return f'({opr}({repr_arrayexpr(args[0])}))'
        else:
            opr = ' ' + opr + ' '
            return f'({opr.join([ repr_arrayexpr(x) for x in args ])})'
...
def parfor_typeinfer(parfor, typeinferer):
    ...
    # XXX
    index_assigns = [ir.Assign(
        ir.Const(1, loc=loc, use_literal_type=False), v, loc) for v in index_vars]
