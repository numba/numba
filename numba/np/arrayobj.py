...
class IntegerArrayIndexer(Indexer):
    ...
    def __init__(self, context, builder, idxty, idxary, size,
                 subspace_allocated, global_ary_idx):
        self.context = context
        self.builder = builder
    ...
    def loop_head(self):
        ...
        if not self.subspace_allocated:
            with builder.if_then(
                ...
                self.builder.store(Constant(self.ll_intp, 0),
                                   self.global_ary_idx)
                builder.branch(self.bb_end)
...
def get_bdcast_idx(context, builder, array_indices):
    ...
    inpty = types.StarArgTuple(
        tuple(types.UniTuple(
            types.intp, count=ary_idx[2].ndim) for ary_idx in array_indices))
    ...
    subspace_shape = context.compile_internal(
        ...
        (cgutils.pack_struct(
            builder,
            tuple([ary_idx[3].shape for ary_idx in array_indices])),)
    )
