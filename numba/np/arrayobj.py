...
def fix_integer_index(context, builder, idxty, idx, size):
    ...
    """
    if idxty.signed:
        ind = context.cast(builder, idx, idxty, types.py_intp)
...
def normalize_index(context, builder, idxty, idx):
    ...
    if isinstance(idxty, types.Array) and idxty.ndim == 0:
        assert isinstance(idxty.dtype, types.BaseInteger)
...
def populate_array(array, data, shape, strides, itemsize, meminfo,
    ...
    *shape* and *strides* can be Python tuples or LLVM arrays.
    ...
    nitems = context.get_constant(types.py_intp, 1)
...
def update_array_info(aryty, array):
    ...
    array.itemsize = context.get_constant(types.py_intp,
...
def getiter_array(context, builder, sig, args):
    ...
    zero = context.get_constant(types.py_intp, 0)
...
def basic_indexing(context, builder, aryty, ary, index_types, indices,
    ...
    """
    zero = context.get_constant(types.py_intp, 0)
    one = context.get_constant(types.py_intp, 1)
    ...
    for indexval, idxty in zip(indices, index_types):
        ...
        elif isinstance(idxty, types.BaseInteger):
...
@lower_builtin(operator.getitem, types.Buffer, types.BaseInteger)
...
class EntireIndexer(Indexer):
    ...
    def __init__(self, context, builder, aryty, ary, dim):
        ...
        self.ary = ary
        self.dim = dim
        self.ll_intp = self.context.get_value_type(types.py_intp)
...
class FancyIndexer(object):
    ...
    def __init__(self, context, builder, aryty, ary, index_types, indices):
        ...
        for indexval, idxty in zip(indices, index_types):
            ...
            if isinstance(idxty, types.SliceType):
                ...
                if isinstance(idxty.dtype, types.BaseInteger):
                    indexer = IntegerArrayIndexer(context, builder,
                                                  idxty, idxary,
                                                  self.shapes[ax])
                elif isinstance(idxty.dtype, types.BaseBoolean):
    ...
    def prepare(self):
        ...
        one = self.context.get_constant(types.py_intp, 1)
...
def numpy_broadcast_to(array, shape):
    ...
    if isinstance(shape, types.BaseInteger):
        ...
        if not isinstance(shape.dtype, types.BaseInteger):
        ...
        if not all([isinstance(typ, types.BaseIntegerLiteral)
                    for typ in shape]):
...
def ol_numpy_broadcast_shapes(*args):
    ...
    for idx, arg in enumerate(args):
        is_int = isinstance(arg, types.BaseInteger)
        is_int_tuple = isinstance(arg, types.UniTuple) and \
            isinstance(arg.dtype, types.BaseInteger)
...
def numpy_broadcast_arrays(*args):
    ...
    for idx, arg in enumerate(args):
        if isinstance(arg, types.ArrayCompatible):
            m = max(m, arg.ndim)
        elif isinstance(arg, (types.Number, types.BaseBoolean,
                              types.BaseTuple)):
            m = max(m, 1)
        else:
...
def fancy_setslice(context, builder, sig, args, index_types, indices):
    ...
    if isinstance(srcty, types.Buffer):
        ...
        len_impl = context.get_function(len, signature(types.py_intp, srcty))
        ...
        def src_getitem(source_indices):
            ...
            getitem_impl = context.get_function(
                operator.getitem,
                signature(src_dtype, srcty, types.py_intp),
            )
...
def vararg_to_tuple(context, builder, sig, args):
    ...
    dims = [context.cast(builder, val, ty, types.py_intp)
            for ty, val in zip(dimtys, dims)]
    # make a tuple
    shape = cgutils.pack_array(builder, dims, dims[0].type)

    shapety = types.UniTuple(dtype=types.py_intp, count=len(dims))
...
def array_transpose_tuple(context, builder, sig, args):
    ...
    ll_intp = context.get_value_type(types.py_intp)
    ...
    np_itemsize = context.get_constant(types.py_intp,
    ...
    for np_ary, ll_ary in zip(np_arys, ll_arys):
        populate_array(np_ary,
                       ...
                       shape=[context.get_constant(types.py_intp, num_axis)],
                       strides=[np_itemsize],
...
def numpy_logspace(start, stop, num=50):
    ...
    if not isinstance(num, (int, types.BaseInteger)):
...
def numpy_geomspace(start, stop, num=50):
    ...
    if any(isinstance(arg, types.BaseComplex) for arg in [start, stop]):
...
def numpy_rot90(m, k=1):
    ...
    if not isinstance(k, (int, types.BaseInteger)):
...
def array_reshape(context, builder, sig, args):
    ...
    shape_itemsize = context.get_constant(types.py_intp,
                                          context.get_abi_sizeof(ll_intp))
    populate_array(shape_ary,
                   data=builder.bitcast(newshape, ll_intp.as_pointer()),
                   shape=[context.get_constant(types.py_intp, shapety.count)],
...
def numpy_resize(a, new_shape):
    ...
    if not ((isinstance(new_shape, types.UniTuple)
             and
             isinstance(new_shape.dtype, types.BaseInteger))
            or
            isinstance(new_shape, types.BaseInteger)):
...
def np_append(arr, values, axis=None):
    ...
    if is_nonelike(axis):
        ...
        if not isinstance(axis, types.BaseInteger):
...
def _change_dtype(context, builder, oldty, newty, ary):
    ...
    old_itemsize = context.get_constant(types.py_intp,
                                        get_itemsize(context, oldty))
    new_itemsize = context.get_constant(types.py_intp,
                                        get_itemsize(context, newty))

    nd = context.get_constant(types.py_intp, newty.ndim)
    ...
    shape_strides_array_type = types.Array(dtype=types.py_intp, ndim=1, layout='C')
    arycls = context.make_array(shape_strides_array_type)

    shape_constant = cgutils.pack_array(builder,
                                        [context.get_constant(types.py_intp,
                                                              newty.ndim)])

    sizeof_intp = context.get_abi_sizeof(context.get_data_type(types.py_intp))
    sizeof_intp = context.get_constant(types.py_intp, sizeof_intp)
    ...
    sig = signature(types.boolean,
                    ...
                    types.py_intp,  # old_itemsize
                    types.py_intp,  # new_itemsize
                    types.int8,  # layout
                    )
...
def np_repeat(a, repeats):
    ...
    if isinstance(a, (types.Array,
        ...
        if isinstance(repeats, types.BaseInteger):
            return np_repeat_impl_repeats_scaler
        elif isinstance(repeats, (types.Array, types.List)):
            if isinstance(repeats.dtype, types.BaseInteger):
...
def array_ndim(context, builder, typ, value):
    res = context.get_constant(types.py_intp, typ.ndim)
    return impl_ret_untracked(context, builder, typ, res)
...
def array_ctypes_data(context, builder, typ, value):
    ...
    res = builder.ptrtoint(res, context.get_value_type(types.py_intp))
    return impl_ret_untracked(context, builder, typ, res)
...
def _call_contiguous_check(checker, context, builder, aryty, ary):
    ...
    tup_intp = types.UniTuple(types.py_intp, aryty.ndim)
    itemsize = context.get_abi_sizeof(context.get_value_type(aryty.dtype))
    check_sig = signature(types.bool_, tup_intp, tup_intp, types.py_intp)
    check_args = [ary.shape, ary.strides,
                  context.get_constant(types.py_intp, itemsize)]
...
def array_record_getattr(context, builder, typ, value, attr):
    ...
    constoffset = context.get_constant(types.py_intp, offset)
    ...
    if isinstance(dtype, types.NestedArray):
        ...
        shape += [context.get_constant(types.py_intp, i) for i in dtype.shape]
        # new strides = recarray strides + strides of the inner nestedarray
        strides = cgutils.unpack_tuple(builder, array.strides, typ.ndim)
        strides += [context.get_constant(types.py_intp, i) for i in dtype.strides]
    ...
    populate_array(rary,
                   ...
                   shape=shape,
                   strides=strides,
                   itemsize=context.get_constant(types.py_intp, datasize),
...
def record_getattr(context, builder, typ, value, attr):
    ...
    if isinstance(elemty, types.NestedArray):
        ...
        dtype = elemty.dtype
        newshape = [context.get_constant(types.py_intp, s) for s in
                    elemty.shape]
        newstrides = [context.get_constant(types.py_intp, s) for s in
                      elemty.strides]
        ...
        populate_array(
            ...
            itemsize=context.get_constant(types.py_intp, elemty.size),
            meminfo=None,
            parent=None,
        )
        res = ary._getvalue()
...
def record_setattr(context, builder, sig, args, attr):
    ...
    if isinstance(elemty, types.NestedArray):
        ...
        cgutils.memcpy(builder, dest, src,
                       context.get_constant(types.py_intp, elemty.nitems))
    else:
...
@lower_builtin('static_getitem', types.Record, types.BaseIntegerLiteral)
...
@lower_builtin('static_setitem', types.Record,
               types.BaseIntegerLiteral, types.Any)
...
def make_nditer_cls(nditerty):
    ...
    class NdIter(cgutils.create_struct_proxy(nditerty)):
        ...
        def init_specific(self, context, builder, arrtys, arrays):
            ...
            main_shape_ty = types.UniTuple(types.py_intp, ndim)
            main_shape = None
            main_nitems = None
            for i, arrty in enumerate(arrtys):
                ...
                main_nitems = context.get_constant(types.py_intp, 1)
            ...
            for arrty, arr in zip(arrtys, arrays):
                if isinstance(arrty, types.Array) and arrty.ndim > 0:
                    sig = signature(types.none,
                                    types.UniTuple(types.py_intp, arrty.ndim),
                                    main_shape_ty)
        ...
        def _make_view(self, context, builder, indices, retty, arrty, arr,
            ...
            shape = context.make_tuple(builder, types.UniTuple(types.py_intp, 0),
                                       ())
            strides = context.make_tuple(builder, types.UniTuple(types.py_intp, 0),
                                         ())
    ...
    class NdIndexIter(cgutils.create_struct_proxy(nditerty)):
        ...
        def init_specific(self, context, builder, shapes):
            ...
            indices = cgutils.alloca_once(builder, zero.type,
                                          size=context.get_constant(types.py_intp,
                                                                    ndim))
    ...
    if array_type.layout == 'C':
            ...
            def init_specific(self, context, builder, arrty, arr):
                ...
                if kind == 'ndenumerate':
                    ...
                    indices = cgutils.alloca_once(
                        builder, zero.type,
                        size=context.get_constant(types.py_intp, arrty.ndim))
        ...
        class FlatIter(cgutils.create_struct_proxy(flatiterty)):
            ...
            def init_specific(self, context, builder, arrty, arr):
                ...
                indices = cgutils.alloca_once(
                    builder, zero.type, size=context.get_constant(types.py_intp,
                                                                  arrty.ndim))
                pointers = cgutils.alloca_once(
                    builder, data.type, size=context.get_constant(types.py_intp,
                                                                  arrty.ndim))
...
@lower_builtin(operator.getitem, types.NumpyFlatType, types.BaseInteger)
...
@lower_builtin(operator.setitem, types.NumpyFlatType, types.BaseInteger,
               types.Any)
...
@lower_builtin(pndindex, types.VarArg(types.BaseInteger))
@lower_builtin(np.ndindex, types.VarArg(types.BaseInteger))
def make_array_ndindex(context, builder, sig, args):
    """ndindex(*shape)"""
    shape = [context.cast(builder, arg, argty, types.py_intp)
...
def make_array_ndindex_tuple(context, builder, sig, args):
    ...
    if ndim > 0:
        ...
        shape = [context.cast(builder, idx, idxty, types.py_intp)
                 for idx in shape]
    else:
        shape = []
...
def _empty_nd_impl(context, builder, arrtype, shapes):
    ...
    itemsize = context.get_constant(types.py_intp, get_itemsize(context, arrtype))

    # compute array length
    arrlen = context.get_constant(types.py_intp, 1)
    ...
    argtypes = signature(mip, arytypeclass, types.py_intp, types.uint32)
...
def _parse_shape(context, builder, ty, val):
    ...
    if isinstance(ty, types.BaseInteger):
        ndim = 1
        passed_shapes = [context.cast(builder, val, ty, types.py_intp)]
    else:
    ...
    zero = context.get_constant_generic(builder, types.py_intp, 0)
    for dim in range(ndim):
...
def numpy_indices(dimensions):
    ...
    if not isinstance(dimensions.dtype, types.BaseInteger):
        msg = 'The argument "dimensions" must be a tuple of integers'
        raise errors.TypingError(msg)
...
def numpy_take(a, indices):
    ...
    if isinstance(a, types.Array) and isinstance(indices, types.BaseInteger):
...
def _arange_dtype(*args):
    ...
    if any(isinstance(a, types.BaseComplex) for a in bounds):
        dtype = types.complex128
    elif any(isinstance(a, types.BaseFloat) for a in bounds):
        dtype = types.float64
    else:
...
def np_arange(start, stop=None, step=None, dtype=None):
    ...
    use_complex = any([isinstance(x, types.BaseComplex)
...
def _as_layout_array(context, builder, sig, args, output_layout):
    ...
    if aryty.ndim == 0:
        ...
        shape = context.get_constant_generic(
            builder, types.UniTuple(types.py_intp, 1), (1,),
        )
        strides = context.make_tuple(builder,
                                     types.UniTuple(types.py_intp, 1),
                                     (ary.itemsize,))
...
def np_cfarray(context, builder, sig, args):
    ...
    if isinstance(shapety, types.BaseTuple):
        ...
        shapety = (shapety,)
        shapes = (shape,)
    shapes = [context.cast(builder, value, fromty, types.py_intp)
...
def _get_seq_size(context, builder, seqty, seq):
    if isinstance(seqty, types.BaseTuple):
        return context.get_constant(types.py_intp, len(seqty))
    elif isinstance(seqty, types.Sequence):
        len_impl = context.get_function(len, signature(types.py_intp, seqty,))
...
def _get_borrowing_getitem(context, seqty):
    ...
    getitem_impl = context.get_function(operator.getitem,
                                        signature(retty, seqty, types.py_intp))
    ...
    def assign(seqty, seq, shapes, indices):
        ...
        if isinstance(seqty, types.Sequence):
            ...
            for i in range(len(seqty)):
                innerty = seqty[i]
                inner = builder.extract_value(seq, i)
                index = context.get_constant(types.py_intp, i)
...
def np_expand_dims(typingctx, a, axis):
    ...
    def codegen(context, builder, sig, args):
        axis = context.cast(builder, args[1], sig.args[1], types.py_intp)
...
def np_concatenate_typer(typingctx, arrays, axis):
    if axis is not None and not isinstance(axis, types.BaseInteger):
...
def np_column_stack(typingctx, tup):
    ...
    def codegen(context, builder, sig, args):
        ...
        axis = context.get_constant(types.py_intp, 1)
...
def _np_hstack(typingctx, tup):
    ...
    def codegen(context, builder, sig, args):
        ...
        if ndim == 0:
            ...
            axis = context.get_constant(types.py_intp, 0)
            return _np_stack_common(context, builder, sig, args, axis)
...
def _np_dstack(typingctx, tup):
    ...
    def codegen(context, builder, sig, args):
        ...
        if ndim == 0:
            ...
            axis = context.get_constant(types.py_intp, 2)
            return _np_stack_common(context, builder, sig, args, axis)
...
def np_array_split(ary, indices_or_sections, axis=0):
    ...
    if isinstance(indices_or_sections, types.BaseInteger):
        def impl(ary, indices_or_sections, axis=0):
        ...
        and all(isinstance(t, types.BaseInteger)
...
def numpy_vsplit(ary, indices_or_sections):
    ...
    if not isinstance(indices_or_sections, (types.BaseInteger, types.Array,
...
def lt_implementation(dtype):
    if isinstance(dtype, types.BaseFloat):
        return lt_floats
    elif isinstance(dtype, types.BaseComplex):
        return lt_complex
    else:
        return default_lt
...
@lower_cast(types.Array, types.BaseFloat)
@lower_cast(types.Array, types.BaseInteger)
@lower_cast(types.Array, types.BaseComplex)
@lower_cast(types.Array, types.BaseBoolean)
...
def type_reshape_unchecked(context):
    def check_shape(shape):
        return (isinstance(shape, types.BaseTuple) and
                all(isinstance(v, types.BaseInteger) for v in shape))
...
def sliding_window_view(x, window_shape, axis=None):
    ...
    if isinstance(window_shape, types.BaseInteger):
        ...
        def get_window_shape(window_shape):
            ...
            isinstance(window_shape.dtype, types.BaseInteger)):
    ...
    elif isinstance(axis, types.BaseInteger):
        @register_jitable
        def get_axis(window_shape, axis, ndim):
            return [
...
def numpy_swapaxes(a, axis1, axis2):
    if not isinstance(axis1, (int, types.BaseInteger)):
        raise errors.TypingError('The second argument "axis1" must be an '
                                 'integer')
    if not isinstance(axis2, (int, types.BaseInteger)):
...
def arr_take_along_axis(arr, indices, axis):
    ...
    if not isinstance(indices.dtype, types.BaseInteger):
    ...
    if is_nonelike(axis):
        ...
        if not isinstance(axis, types.BaseIntegerLiteral):
    ...
    if isinstance(x, types.Number):
        if isinstance(x, types.BaseInteger):
            ...
            def impl(x, copy=True, nan=0.0):
                ...
                return x
        elif isinstance(x, types.BaseComplex):
            def impl(x, copy=True, nan=0.0):
        ...
        if isinstance(x.dtype, types.BaseInteger):
            # Integers do not have nans or infs
            def impl(x, copy=True, nan=0.0):
                return x
        elif isinstance(x.dtype, types.BaseFloat):
            def impl(x, copy=True, nan=0.0):
                ...
                return output
        elif isinstance(x.dtype, types.BaseComplex):
            def impl(x, copy=True, nan=0.0):
                x_ = np.asarray(x)
                output = np.copy(x_) if copy else x_
