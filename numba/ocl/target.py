...
ocl_data_model_manager = _init_data_model_manager()
...
class OCLTargetContext(BaseContext):
    ...
    def generate_kernel_wrapper(self, func, argtypes):
        ...
        wrapper_module = self.create_module("ocl.kernel.wrapper")
        wrappername = 'oclPy_{name}'.format(name=func.name)
    ...
    def declare_function(self, module, fndesc):
        ret = super(OCLTargetContext, self).declare_function(module, fndesc)
        # XXX: Refactor fndesc instead of this special case
        if fndesc.llvm_func_name.startswith('oclpy_devfn'):
...
class OCLCallConv(MinimalCallConv):
