"""
A OpenCL ND Array is recognized by checking the __ocl_memory__ attribute
...
def is_ocl_ndarray(obj):
    "Check if an object is a OpenCL ndarray"
...
def require_ocl_ndarray(obj):
    "Raises ValueError is is_ocl_ndarray(obj) evaluates False"
    if not is_ocl_ndarray(obj):
        raise ValueError('require an ocl ndarray object')
...
class DeviceNDArrayBase(object):
    ...
    """
    __ocl_memory__ = True
    __ocl_ndarray__ = True     # There must be gpu_data attribute
    ...
    def copy_to_device(self, ary, stream=0):
        ...
        If `ary` is a OpenCL memory, perform a device-to-device transfer.
    ...
    def copy_to_host(self, ary=None, stream=0):
        ...
        If a OpenCL ``stream`` is given, then the transfer will be made
...
class DeviceNDArray(DeviceNDArrayBase):
    ...
    def getitem(self, item, stream=0):
        """Do `__getitem__(item)` with OpenCL stream
        """
...
class MappedNDArray(DeviceNDArrayBase, np.ndarray):
    """
    A host array that uses OpenCL mapped memory.
    """
