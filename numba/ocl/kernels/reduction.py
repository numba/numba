"""
A library written in OpenCL Python for generating reduction kernels
"""
...
def reduction_template(binop, typ, blocksize):
    ...
    blocksize : int
        The OpenCL block size (thread per block)
    """
    ...
    binop = ocl.jit((typ, typ), device=True)(binop)

    # Compile reducer
    @ocl.jit((typ[:], typ[:], intp, intp))
    def reducer(inp, out, nelem, ostride):
        tid = ocl.threadIdx.x
        i = ocl.blockIdx.x * (blocksize * 2) + tid
        gridSize = blocksize * 2 * ocl.gridDim.x

        # Blocks perform most of the reduction within shared memory, in the
        # sdata array
        sdata = ocl.shared.array(sdatasize, dtype=typ)
        ...
        if tid == 0:
            out[ocl.blockIdx.x * ostride] = sdata[0]
...
def _get_copy_strides_kernel():
    ...
    if _get_copy_strides_kernel_cache is None:
        ...
        def copy_strides(arr, n, stride, tpb):
            sm = ocl.shared.array(1, dtype=uint32)
            i = ocl.threadIdx.x
            base = 0
            if i == 0:
                sm[0] = 0
...
class Reduce(object):
    ...
    def __init__(self, binop):
        ...
        :param binop: A function to be compiled as a OpenCL device function that
                      will be used as the binary operation for reduction on a
                      OpenCL device. Internally, it is compiled using
                      ``ocl.jit(signature, device=True)``.
        """
        self._kernels = {}
    ...
    def _execute_reduction(self, dary, size, stream, init=0):
        ...
        while size >= 2:
            ...
            if diffsize > 0:
                ...
                ocl.driver.device_to_device(dst, src, diffsize * itemsize,
                                             stream=stream)
                size += diffsize
    ...
    def _precompile_kernels(self, nbtype):
        ...
        if nbtype not in self._cached_types:
            ...
            # initial value.
            binop = ocl.jit((nbtype, nbtype), device=True)(self._binop)

            @ocl.jit((nbtype[:], nbtype))
    ...
    def __call__(self, arr, res=None, size=None, init=0, stream=0):
        ...
        :param stream: Optional OpenCL stream in which to perform the reduction.
        ...
        darr, _ = ocl.devicearray.auto_device(arr, stream=stream)
