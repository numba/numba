...
class TestHostAlloc(OCLTestCase):
    ...
    def test_host_alloc_pinned(self):
        ary = ocl.pinned_array(10, dtype=np.uint32)
        ary.fill(123)
        self.assertTrue(all(ary == 123))
        devary = ocl.to_device(ary)
    ...
    def test_host_alloc_mapped(self):
        ary = ocl.mapped_array(10, dtype=np.uint32)
        ary.fill(123)
        self.assertTrue(all(ary == 123))
