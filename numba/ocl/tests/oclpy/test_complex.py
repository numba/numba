...
from numba import ocl, types, utils, numpy_support
...
def compile_scalar_func(pyfunc, argtypes, restype):
    ...
    device_func = ocl.jit(restype(*argtypes), device=True)(pyfunc)
    ...
    kernel = ocl.jit(tuple(kernel_types))(kernel_func)
...
class BaseComplexTest(object):
    ...
    def run_func(self, pyfunc, sigs, values, ulps=1):
        for sig in sigs:
            ...
            oclfunc = compile_scalar_func(pyfunc, sig.args, sig.return_type)
            ok_values = []
            expected_list = []
            for args in values:
