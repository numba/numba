...
from numba import ocl, int32, complex128
from numba.ocl.testing import unittest


def culocal(A, B):
    C = ocl.local.array(1000, dtype=int32)
...
def culocalcomplex(A, B):
    C = ocl.local.array(100, dtype=complex128)
    for i in range(C.shape[0]):
        C[i] = A[i]
    for i in range(C.shape[0]):
        B[i] = C[i]
...
def culocal1tuple(A, B):
    C = ocl.local.array((5,), dtype=int32)
    for i in range(C.shape[0]):
        C[i] = A[i]
    for i in range(C.shape[0]):
        B[i] = C[i]
...
class TestOclLocalMem(unittest.TestCase):
    def test_local_array(self):
        jculocal = ocl.jit('void(int32[:], int32[:])')(culocal)
    ...
    def test_local_array_1_tuple(self):
        ...
        """
        jculocal = ocl.jit('void(int32[:], int32[:])')(culocal1tuple)
    ...
    def test_local_array_complex(self):
        ...
        jculocalcomplex = ocl.jit(sig)(culocalcomplex)
