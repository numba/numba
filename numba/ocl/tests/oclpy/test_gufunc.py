...
class TestOCLGufunc(unittest.TestCase):
    ...
    def test_gufunc(self):
        ...
        matrix_ct = 1001 # an odd number to test thread/block division in OCL
        A = np.arange(matrix_ct * 2 * 4, dtype=np.float32).reshape(matrix_ct, 2,
                                                                   4)
        B = np.arange(matrix_ct * 4 * 5, dtype=np.float32).reshape(matrix_ct, 4,
                                                                   5)
    ...
    def test_gufunc_hidim(self):
        ...
        matrix_ct = 100 # an odd number to test thread/block division in OCL
    ...
    def test_gufunc_stream(self):
        ...
        dC = ocl.device_array(shape=(1001, 2, 5), dtype=A.dtype, stream=stream)
    ...
    def test_nopython_flag(self):
        ...
        guvectorize([void(float32[:], float32[:])], '(x)->(x)', target='ocl',
                    nopython=True)(foo)

        # nopython = False is bad
        with self.assertRaises(TypeError) as raises:
            guvectorize([void(float32[:], float32[:])], '(x)->(x)',
                        target='ocl', nopython=False)(foo)
    ...
    def test_invalid_flags(self):
        ...
        with self.assertRaises(TypeError) as raises:
            guvectorize([void(float32[:], float32[:])], '(x)->(x)',
                        target='ocl', what1=True, ever2=False)(foo)
