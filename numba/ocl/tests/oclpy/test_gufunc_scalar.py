...
import numpy as np
from numba import guvectorize, ocl
...
class TestGUFuncScalr(unittest.TestCase):
    ...
    #        - the function takes an array of n-element and output a scalar.
        ...
        @guvectorize(['void(int32[:], int32[:])'], '(n)->()', target='ocl')
        def sum_row(inp, out):
            tmp = 0.
        ...
        # But, broadcasting on OpenCL arrays is not supported.

        inp = np.arange(300, dtype=np.int32).reshape(100, 3)

        # invoke on OpenCL with manually managed memory
        out1 = np.empty(100, dtype=inp.dtype)
        out2 = np.empty(100, dtype=inp.dtype)

        dev_inp = ocl.to_device(
            inp)                 # alloc and copy input data
        dev_out1 = ocl.to_device(out1, copy=False)   # alloc only
