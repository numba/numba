...
class TestReduction(unittest.TestCase):
    ...
    def test_prod_reduce(self):
        prod_reduce = ocl.reduce(lambda a, b: a * b)
    ...
    def test_max_reduce(self):
        max_reduce = ocl.Reduce(lambda a, b: max(a, b))
    ...
    def test_result_on_device(self):
        A = (np.arange(10, dtype=np.float64) + 1)
        got = ocl.to_device(np.zeros(1, dtype=np.float64))
        expect = A.sum()
