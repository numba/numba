...
class IsCloseTests(TestCase):
    ...
    def test_identical_infinite(self):
        ...
        self.assertIsClose(self.INF, self.INF)
        self.assertIsClose(self.INF, self.INF, abs_tol=0.0)
        self.assertIsClose(self.NINF, self.NINF)
        self.assertIsClose(self.NINF, self.NINF, abs_tol=0.0)

    def test_inf_ninf_nan(self):
        # these should never be close (following IEEE 754 rules for equality)
        not_close_examples = [(self.NAN, self.NAN),
                              (self.NAN, 1e-100),
                              (1e-100, self.NAN),
                              (self.INF, self.NAN),
                              (self.NAN, self.INF),
