...
class TestExtensionTypeMixin(TestCase):
    """Tests the use of the types.ExtensionTypeMixin
    """
    ...
    def test_mixin_basic(self):
        ...
        # use the extension type mixin
        class FooType(types.ExtensionTypeMixin, types.Type):
            def __init__(self):
        ...
        # Set up an overload which will accept all types irrespective of whether
        # they are from Numba's closed type system
        ...
        def ol_false_if_not_array_closed_system(a):
            if not a.is_extension_type:  # guard
                if isinstance(a, types.Array):
                    return lambda a : True
                else:
                    return lambda a : False
    ...
    def test_mixin_against_real_example(self):
        # See issue #4970, this checks that unicode equals now ignores extension
        # types.
        ...
        def foo_eq(a, b):
            if a == FooType():
                return lambda a, b: "RAN CUSTOM OVERLOAD"

        @njit
        def f(a):
            return a == "A"

        self.assertEqual("RAN CUSTOM OVERLOAD", f(Foo()))
