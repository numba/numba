...
class TestUnaryOps(unittest.TestCase):
    ...
    @test_support.checkSkipFlag("Wrapper REALLY blowing up.")
    ...
    @test_support.checkSkipFlag("Encountering weird issue in transformer.")
    def test_unary_not_pred(self):
        test_fn = jit(argtypes=(bool_,), restype=bool_)(unary_not_pred)
        for test_val in True, False:
            self.assertEqual(test_fn(test_val), 0 if test_val else 1)
            self.assertEqual(test_fn(test_val), unary_not(test_val))

    @test_support.checkSkipFlag("Wrapper blowing up.")
