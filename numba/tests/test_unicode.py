...
class TestUnicode(BaseTest):
    ...
    def test_islower(self):
        ...
        # Samples taken from CPython testing:
        # https://github.com/python/cpython/blob/3.6/Lib/test/test_unicode.py#L571-L584
        cpython = ['\u2167', '\u2177', '\U00010401', '\U00010427', '\U00010429',
                   '\U0001044E', '\U0001F40D', '\U0001F46F']
        fourxcpy = [x * 4 for x in cpython]

        for a in UNICODE_EXAMPLES + lowers + [""] + extras + cpython + fourxcpy:
            args = [a]
            self.assertEqual(pyfunc(*args), cfunc(*args),
                             msg='failed on {}'.format(args))
    ...
    def test_lower(self):
        ...
        # Samples taken from CPython testing:
        # https://github.com/python/cpython/blob/3.6/Lib/test/test_unicode.py#L748-L758
        ...
        # https://github.com/python/cpython/blob/3.6/Lib/test/test_unicode.py#L759-L768
        sigma = ['\u03a3', '\u0345\u03a3', 'A\u0345\u03a3', 'A\u0345\u03a3a', '\u03a3\u0345 ', '\U0008fffe', '\u2177']
        for a in UNICODE_EXAMPLES + [""] + extras + cpython + sigma:
            args = [a]
            self.assertEqual(pyfunc(*args), cfunc(*args),
                             msg='failed on {}'.format(args))
