...
from numba.extending import (
    box, NativeValue, register_model, typeof_impl, unbox,
...
@contextmanager
def check_numba_allocations(
        self, create_tracked_objects, extra_allocations=0, refcount_changes={}
):
    ...
    try:
        ...
        refcounts_after = {
            k: getrefcount(v) for k, v in track_refcounts.items()}
        del track_refcounts
        ...
        self.assertEqual(
            after.alloc - before.alloc - extra_allocations,
            after.free - before.free
        )
    finally:
        # try to trigger any __del__ that might not been called b/c of
        # exceptions, otherwise these show up randomly later
        gc.collect()
...
class PassThruContainerTest(TestCase):
    def test_identity(self):
        ...
        with check_numba_allocations(
                self, (lambda: dict(c=PassThruContainer(obj)))) as (c,):
    ...
    def test_eq(self):
        ...
        def container_eq(c1, c2, c3, c4):
            ...
            return (
                a, A, c1 == c1, c1 == c2, c1 == c3, c1 == c4, c2 == c2,
                c2 == c3, c2 == c4, c3 == c3, c3 == c4, c4 == c4
            )
        ...
        with check_numba_allocations(
            ...
            a, A, r1, r2, r3, r4, r5, r6, r7, r8, r9, r10 = container_eq(
                c1, c2, c3, c4)
            ...
            self.assertEqual(
                ...
                (c1 == c1, c1 == c2, c1 == c3, c1 == c4, c2 == c2, c2 == c3,
                 c2 == c4, c3 == c3, c3 == c4, c4 == c4)
            )
...
class MyPassThruTest(TestCase):
    def test_forget(self):
        with check_numba_allocations(
                self, (lambda: dict(o=MyPassThru()))
        ) as (o,):
            _id = forget(o)
            self.assertEqual(
                _id, id(o.something_not_boxable) & (2**MACHINE_BITS - 1)
            )
            del o
    ...
    def test_list_identity(self):
        with check_numba_allocations(
                self,
                (lambda: dict(x=MyPassThru(), y=MyPassThru(), z=MyPassThru()))
        ) as (x, y, z):
            l = [x, y, z]
            l2 = identity(l)
            self.assertIs(l, l2)
    ...
    def test_list_create(self):
        with check_numba_allocations(
                self,
                (lambda: dict(x=MyPassThru(), y=MyPassThru()))
        ) as (x, y):
...
def unbox_passthru_complex(typ, obj, context):
    ...
    pass_thru.passthru_attr = context.unbox(
        pass_thru_type, passthru_attr
    ).value
    context.pyapi.decref(passthru_attr)

    # We want to have lists of PassThruComplexType. We must copy the list as
    # nopython lists have a cleanup functions that will not get called for
    # list members (ie forwarding like
    ...
    py_list_obj = context.pyapi.unserialize(
        context.pyapi.serialize_object(list)
    )
    list_attr_copy = context.pyapi.call_function_objargs(
        py_list_obj, [list_attr]
    )
    ...
    # should have done this more often, errors most likely erased by subsequent
    # pyapi call anyway ...
    is_error = cgutils.is_not_null(
        context.builder, context.pyapi.err_occurred()
    )
...
def box_passthru_complex(typ, val, context):
    ...
    # just unwrap the parent Python object, no attempt at reflecting attribute
    # mutations back incref pass_thru.parent, then box pass_thru.parent (which
    # will decref again), finally decref val
    context.context.nrt.incref(
        context.builder, pass_thru_type, pass_thru.parent
    )
...
class PassThruComplexTest(TestCase):
    ...
    def test_attr_access(self):
        with check_numba_allocations(
            ...
            value, x2, (y2, z2) = attr_access(
                o, 'int_attr', 'passthru_attr', 'list_attr'
            )
