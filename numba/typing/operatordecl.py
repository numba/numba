...
class Operator_comparison(ConcreteTemplate):
    cases = [
...
comparison_operators = ['eq', 'ne', 'lt', 'le', 'gt', 'ge', 'and_', 'or_', 'xor']

for op in comparison_operators:
    op_type = type('Operator_' + op, (Operator_comparison,), {'key':getattr(operator, op)})
    setattr(OperatorModuleAttribute, 'resolve_' + op,  create_resolve_method(op_type))
    builtin_global(getattr(operator, op), types.Function(op_type))
