...
def runWaveStencil(p, c, f, r, t, n, s2, s4, s):
    ...
    f[1:s-1, 0:1] = (2.0 * c[1:s-1, 0:1] + (r-1.0) * p[1:s-1, 0:1] + 2.0*r*r*(c[1:s-1, 1:2] + c[2:s, 0:1] + c[0:s-2, 0:1] - 3.0 * c[1:s-1, 0:1])) / (1.0+r) # Y:1
    f[1:s-1, s-1:s] = (2.0 * c[1:s-1, s-1:s] + (r-1.0) * p[1:s-1, s-1:s] + 2.0*r*r*(c[1:s-1, s-2:s-1] + c[2:s, s-1:s] + c[0:s-2, s-1:s] - 3.0 * c[1:s-1, s-1:s])) / (1.0+r) # Y:s
    f[0:1, 1:s-1] = (2.0 * c[0:1, 1:s-1] + (r-1.0) * p[0:1, 1:s-1] + 2.0*r*r*(c[1:2, 1:s-1] + c[0:1, 2:s] + c[0:1, 0:s-2] - 3.0 * c[0:1, 1:s-1])) / (1.0+r) # X:1
    f[s-1:s, 1:s-1] = (2.0 * c[s-1:s, 1:s-1] + (r-1.0) * p[s-1:s, 1:s-1] + 2.0*r*r*(c[s-2:s-1, 1:s-1] + c[s-1:s, 2:s] + c[s-1:s, 0:s-2] - 3.0 * c[s-1:s, 1:s-1])) / (1.0+r) # Y:s
