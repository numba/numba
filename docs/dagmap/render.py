...
Dagmap = collections.namedtuple("Dagmap", ["version", "meta", "style", "tasks"])
...
def to_graphviz(dagmap):
    ...
    for task in dagmap.tasks:
        G.node(
            name=task["id"], label=task["label"], tooltip=task["description"].strip()
        )
...
def main(argv):
    ...
    parser.add_argument(
        "-t", "--template", default="template.html", help="HTML rendering template"
    )
    parser.add_argument("input", metavar="INPUT", type=str, help="YAML input filename")
